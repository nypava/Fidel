...
H_VOWELS = ["a","u","i","⨳","e","","o","ua"] # Vowels used for "ሀ"
A_VOWELS = ["a","u","i","⨳","ie","e","o","ua"] # Vowels used for "አ"
...
def alphabet_dictionary(symbol:bool) -> Dict:
    ...
    for alphabet in range(26): # Loop that add pair of letter | consonants and vowels
        for vowel in range(8):
            cons_vow_pair[index].append(f"{AM_CONSONANTS[alphabet]}{AM_VOWELS[vowel]}")
        index += 1
   
    for alphabet in range(2): # Loop that add pair of letter "አሀ" special case
        for vowel in range(8):
            if AH_ALPHABET[alphabet] == "":
                cons_vow_pair[index].append(f"{AH_ALPHABET[alphabet]}{A_VOWELS[vowel]}")
                continue
            cons_vow_pair[index].append(f"{AH_ALPHABET[alphabet]}{H_VOWELS[vowel]}")
        index += 1
   
    for index1 in range(28): # Final loop that pair Amharic alpha to consonant and vowel pair
